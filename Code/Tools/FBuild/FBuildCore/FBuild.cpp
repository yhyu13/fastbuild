    ...
    , m_ImportedEnvironmentVars( 0, true )
{
    ...
    const uint32_t ip = Network::GetHostIPFromName( m_HostName );
...
/*virtual*/ bool FBuild::Build( Node * nodeToBuild )
{
    ...
    if ( m_Options.m_AllowDistributed )
    {
        ...
        if ( workers.IsEmpty() )
        {
            // TODO @yuhang : find why m_AllowDistributed prevents PCH for both local and remote workers
            // Solved : ObjectNode::DoBuildWithPreProcessor2 has turned PCH off when distributing. FB remote worker cannot use PCH file, and obj generated by remote worker cannot use PCH for further process.
            // So, FB at current stage cannot benefit from PCH, which result in 3 times slower in Unreal Engine full re-build which renders distributed compiling useless.
            // It would require a total re-desiging a virtual file system for the remote worker to use the PCH files.
            FLOG_WARN( "No workers available - Distributed compilation disabled" );
            m_Options.m_AllowDistributed = false;
        }
        else
        {
            // TODO @yuhang : disable local worker (i.e. worker with same I.P as local mechaine) from workers list, so that we won't distribute works exceeding available cores.
            Array< AString > trim_workers;
            for(const auto& ip : workers)
            {
                if (ip != m_IPAddress)
                {
                    trim_workers.Append(ip);
                    OUTPUT( "Discovering worker : %s in pool\n", ip.Get());
                }
                else
                {
                    // TODO @yuhang : would removal of local worker be contradictory to -forceremote option? Is it the reason behind the original logic of not removing the local worker?
                    OUTPUT( "Ignore worker : %s because it is the local host machine\n", ip.Get());
                }
            }
            OUTPUT( "Distributed Compilation : %u Workers in pool '%s'\n", (uint32_t)trim_workers.GetSize(), m_WorkerBrokerage.GetBrokerageRootPaths().Get() );
            m_Client = FNEW( Client( trim_workers, m_Options.m_DistributionPort, settings->GetWorkerConnectionLimit(), m_Options.m_DistVerbose ) );
        }
    }
